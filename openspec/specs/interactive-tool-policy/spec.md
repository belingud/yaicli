...
Define how interactive requests determine tool availability so mode switches affect only the current request, `exec` requests never advertise tools, and disallowed tool calls are not executed or persisted.
